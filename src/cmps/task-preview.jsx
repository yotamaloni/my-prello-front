...
class _TaskPreview extends React.Component {
    ...
    render() {
        const { task, groupId, boardId, board } = this.props
        ...
        return (
            <div>
                <NavLink className="clean-link" to={`/board/${boardId}/${groupId}/${task.id}`}>
                    <section className="task-preview">
                        {/* <div className='edit-btn'><button onClick={()=>{  removeTask (boardId, groupId, task.id)}} ><EditIcon sx={{ fontSize: 16 }}></EditIcon></button></div> */}
                        {(style?.imgUrl) &&
                            <React.Fragment >
                        ...
                        {(description || comments?.length || dueDate || members?.length) &&
                            <React.Fragment >
                                <TaskPreviewInfo  description={description} comments={comments} dueDate={dueDate} members={members} />
                            </React.Fragment>
